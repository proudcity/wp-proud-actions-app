...
$rootScope.$state=$state,$rootScope.$stateParams=$stateParams,$rootScope.currentSection="",$rootScope.currentRoute="",$rootScope.mapboxAccessToken=_.get(Proud,"settings.global.mapbox.token")||"pk.eyJ1IjoiYWxiYXRyb3NzZGlnaXRhbCIsImEiOiI1cVUxbUxVIn0.SqKOVeohLfY0vfShalVDUw",$rootScope.mapboxMap=_.get(Proud,"settings.global.mapbox.map")||"albatrossdigital.lpkdpcjb",$rootScope.externalLinkWindow=_.get(Proud,"settings.global.external_link_window")||!0,
...
// Api Options
// ---------------------------
$rootScope.apiUrl=_.get(Proud,"settings.proud_actions_app.global.api_path")||"https://west-carrollton-oh.proudcity.com/wp-json/wp/v2/",$rootScope.paymentUrl=_.get(Proud,"settings.proud_actions_app.global.payment_url")||"http://demo.helmcivic.com/invoice-example",//$rootScope.proudcityApi + 'invoice-example';
...
StripeCheckoutProvider.defaults({key:_.get(Proud,"settings.proud_actions_app.global.payment.stripe_key")||""})}]).directive("app311Wrap",function($window,$rootScope){return{restrict:"AC",templateUrl:"views/apps/311App/app-311-wrap.html",transclude:!0,link:function($scope,$element,$attrs){
// Display options
// ---------------------------
// Expand categories?
...
// Push objects
_.forEach(active311Items,function(item,key){switch(key){case"faq":item&&"0"!==item&&(menu.push({title:"Get Answers",state:"faq",icon:"fa-question-circle"}),default311Url||(default311Url="/city/answers"));break;case"payments":item&&"0"!==item&&(menu.push({title:"Make a Payment",state:"payments",icon:"fa-credit-card"}),default311Url||(default311Url="/city/payments"));break;case"report":if(item&&"0"!==item){var item={title:"Report an Issue",icon:"fa-exclamation-triangle"};"link"==issue.service?item.url=issue.link_report:item.state="report",menu.push(item),default311Url||(default311Url="/city/report")}break;case"status":if(item&&"0"!==item){var item={title:"Check Status",icon:"fa-wrench"};"link"==issue.service?item.url=issue.link_status:item.state="report",menu.push(item),default311Url||(default311Url="/city/status")}}})}};self.getDefaultUrl=function(appId){return initMenu(appId),default311Url},self.getMenu=function(appId){return initMenu(appId),menu},
// Just return self
