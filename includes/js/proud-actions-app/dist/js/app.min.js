...
// listen for a click
$element.on("click",function(){jQuery("#"+$scope.openReveal).foundation("reveal","open")})}}}),angular.module("311AppParent").filter("termById",function(){return function(items,id){for(var i=0;i<items.length;i++)if(parseInt(items[i].id)==parseInt(id))return items[i];return null}}).filter("termBySlug",function(){return function(items,slug){for(var i=0;i<items.length;i++)if(items[i].slug===slug)return items[i];return null}}).filter("termsNoParent",function(){return function(items){for(var filtered=[],i=0;i<items.length;i++)0==items[i].parent&&filtered.push(items[i]);return filtered}}).filter("termsByParent",function(){return function(items,id){for(var filtered=[],i=0;i<items.length;i++)items[i].parent==id&&filtered.push(items[i]);return filtered}}),angular.module("311App",["wordpressService","seeClickFixService","paymentService","ui.router","ngSanitize","ngAnimate","stripe.checkout","ngTouch","scrollTo"]).run(["$rootScope","$state","$stateParams","$window","$location",function($rootScope,$state,$stateParams,$window,$location){
// Api Options
// ---------------------------
