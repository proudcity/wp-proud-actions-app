...
data.pollingLocations="array"==typeof data.pollingLocations?data.pollingLocations:[];var combinedLocations=data.pollingLocations.concat(data.earlyVoteSites);if(void 0!=combinedLocations&&combinedLocations.length>0){var addr,address;for(var i in combinedLocations)addr=combinedLocations[i].address,address=addr.line1+", "+(void 0!=addr.line2?addr.line2+", ":"")+addr.city+", "+addr.state+" "+addr.zip,$scope.addMarker(address,"Polling place: "+addr.locationName,i>=data.pollingLocations.length?"green":"orange");$scope.directionsLink="//maps.google.com/maps/dir/"+encodeURIComponent($state.params.latlng)+"/"+encodeURIComponent(address),$scope.address=combinedLocations[0].address,$scope.hours=combinedLocations[0].pollingHours.replace(/\n/g,"<br>");var arrHrs=$scope.hours.split("<br>");$scope.shortHours=arrHrs.length>3?[arrHrs[0],arrHrs[1],arrHrs[2]].join("<br>"):null,$scope.longHours=!1,$scope.sources=combinedLocations[0].sources,$scope.isEarly=i>=data.pollingLocations.length}$scope.links=data.state[0].electionAdministrationBody,$scope.disclaimer=!1,$scope.toggleDisclaimer=function(){$scope.disclaimer=!$scope.disclaimer},$scope.getDirections=function($e){$e.preventDefault(),window.open($scope.directionsLink)},$scope.toggleHours=function($e){$e.preventDefault(),$scope.longHours=!$scope.longHours},$scope.mapScroll=function($e){$e.preventDefault(),document.getElementById("google-map").scrollIntoView(!0)}}}).state("city.vote.location.issues",{url:"/issues",templateUrl:"views/apps/311App/vote/vote-issues.html",data:{title:"Issues",// Sets meta title
description:"About the about",// Sets different meta description
keywords:"About, this, page",// Sets different meta keywords,
doScroll:!1,// no scroll on route change
undoMainToggle:!0},controller:function($scope,$rootScope,$state,$filter,data){$scope.contests=$filter("filter")(data.contests,{type:"Referendum"}),$scope.active=null,$scope.setActive=function(val,$e){console.log(val),$e.preventDefault(),$scope.active=val}}}).state("city.vote.location.candidates",{url:"/candidates",templateUrl:"views/apps/311App/vote/vote-candidates.html",data:{title:"Candidates",// Sets meta title
description:"About the about",// Sets different meta description
keywords:"About, this, page",// Sets different meta keywords,
doScroll:!1,// no scroll on route change
undoMainToggle:!0},controller:function($scope,$rootScope,$state,$filter,data){$scope.contests=$filter("filter")(data.contests,{type:"General"}).slice().reverse(),$scope.googleSearch=function(name,loc,$e){$e.preventDefault(),console.log(name),window.open("//google.com/search?q="+encodeURIComponent(name+" "+loc))}}})}])}(jQuery);