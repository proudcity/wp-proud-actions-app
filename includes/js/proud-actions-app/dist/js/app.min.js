...
$rootScope.$state=$state,$rootScope.$stateParams=$stateParams,$rootScope.currentSection="",$rootScope.currentRoute="",$rootScope.mapboxAccessToken=_.get(Proud,"settings.proud_actions_app.mapbox_access_token")||"pk.eyJ1IjoiYWxiYXRyb3NzZGlnaXRhbCIsImEiOiI1cVUxbUxVIn0.SqKOVeohLfY0vfShalVDUw",$rootScope.mapboxMap=_.get(Proud,"settings.proud_actions_app.mapbox_map")||"albatrossdigital.lpkdpcjb",$rootScope.externalLinkWindow=_.get(Proud,"settings.global.external_link_window")||!0,
...
$rootScope.currentYPosition()<300&&$("html, body").animate({scrollTop:$("#wrapper-311").offset().top-70},300),toggleMain($scope.mainToggle,$scope.mainToggleForce,select)})}}}).directive("menuClick",function($window,$browser){return{restrict:"A",link:function($scope,$element,$attrs){$element.on("click",function(event){return jQuery(".app-wrap").toggleClass("show-nav"),$attrs.menuClick&&"return-false"==$attrs.menuClick?(event.stopPropagation(),!1):void 0})}}}).directive("href",function($rootScope){return{compile:function(element){if(console.log($rootScope),$rootScope.externalLinkWindow){var a=new RegExp("/"+window.location.host+"/"),href=element.attr("href");void 0!=href&&href&&!a.test(element.href)&&(console.log(href),element.attr("target","_blank"))}}}}),angular.module("311App").provider("Menu311",function(){
// Init Proud   
