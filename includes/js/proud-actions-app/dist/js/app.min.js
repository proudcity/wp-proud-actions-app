...
var path=$location.path(),subPath=path.indexOf("city/")>=0?path.substring(path.indexOf("city/")+5):path;$window.ga("send","pageview",{page:subPath,title:toState.data&&toState.data.title?toState.data.title:"FILLME!"}),$window.ga("send",{hitType:"event",eventCategory:"ActionsApp",eventAction:"navigation",eventLabel:path})}fromState.name&&fromState.name.length&&(!toState.data||!toState.data||!toState.data.skipScroll)}),
...
// Api Options
// ---------------------------
$rootScope.apiUrl=_.get(Proud,"settings.proud_actions_app.global.api_path")||"https://example.proudcity.com/wp-json/wp/v2/",$rootScope.paymentUrl=_.get(Proud,"settings.proud_actions_app.global.payment_url")||"https://demo-api.proudcity.com/api/proudcity/invoice-example",//$rootScope.proudcityApi + 'invoice-example';
...
// Global options
// ---------------------------
$rootScope.activeCity=_.get(Proud,"settings.global.location.city")||"Huntsville",$rootScope.activeState=_.get(Proud,"settings.global.location.state")||"Alabama",$rootScope.location=_.get(Proud,"settings.global.location")||{lat:0,lng:0},$rootScope.mapboxAccessToken=_.get(Proud,"settings.global.mapbox.token")||"pk.eyJ1IjoicHJvdWRjaXR5IiwiYSI6ImNpa2JsN2h4eTBtM3N2NGt1ZWltdmRwM3gifQ.yMFhYzprJEYIecChP-rJAg",$rootScope.mapboxMap=_.get(Proud,"settings.global.mapbox.map")||"https://api.mapbox.com/v3/mapbox.streets.json",
...
$rootScope.toggleMain=function(toggleClass,toggleType,select){switch(select=select||"",toggleType){case"remove":jQuery("#main-311"+select).removeClass(toggleClass);break;case"add":jQuery("#main-311"+select).addClass(toggleClass);break;default:jQuery("#main-311"+select).toggleClass(toggleClass)}}}]).config(["$locationProvider","$stateProvider","$urlRouterProvider","StripeCheckoutProvider",function($locationProvider,$stateProvider,$urlRouterProvider,StripeCheckoutProvider){
// set location provider as regular urls
// $locationProvider.html5Mode(true);
StripeCheckoutProvider.defaults({key:_.get(Proud,"settings.proud_actions_app.global.payment.stripe_key")||"sk_test_0sXDrBwvUc9ZPZHeq89HSxOV"})}]).directive("app311Wrap",function($window,$rootScope){return{restrict:"AC",templateUrl:"views/apps/311App/app-311-wrap.html",transclude:!0,link:function($scope,$element,$attrs){
// Display options
// ---------------------------
// Expand categories?
...
$rootScope.currentYPosition()<300&&$("html, body").animate({scrollTop:$("#wrapper-311").offset().top-70},300),$rootScope.toggleMain($scope.mainToggle,$scope.mainToggleForce,select)})}}}).directive("menuClick",function($window,$browser){return{restrict:"A",link:function($scope,$element,$attrs){$element.on("click",function(event){return jQuery(".app-wrap").toggleClass("show-nav"),$attrs.menuClick&&"return-false"==$attrs.menuClick?(event.stopPropagation(),!1):void 0})}}}).directive("href",function($rootScope){return{restrict:"A",link:function($scope,$element,$attrs){if($rootScope.externalLinkWindow){var a=new RegExp("/"+window.location.host+"/"),href=void 0!=$attrs.ngHref?$attrs.ngHref:$attrs.href;void 0==href||-1==href.indexOf("http")||a.test(href)||$element.attr("target","_blank")}}}}),angular.module("311App").provider("Menu311",function(){
// Init Proud   
var self=this,active311Items=null,default311Url=null,menu=[],initMenu=function(appId){
// Only run once
if(!active311Items){active311Items=_.get(Proud,"settings.proud_actions_app.instances."+appId+".active_tabs")||{faq:!0,payments:!0,report:!0,status:!0};var issue=_.get(Proud,"settings.proud_actions_app.global.issue")||{service:"seeclickfix"};
// Push objects
_.forEach(active311Items,function(item,key){switch(key){case"faq":item&&"0"!==item&&(menu.push({title:"Get Answers",state:"faq",icon:"fa-question-circle"}),default311Url||(default311Url="/city/answers"));break;case"payments":item&&"0"!==item&&(menu.push({title:"Make a Payment",state:"payments",icon:"fa-credit-card"}),default311Url||(default311Url="/city/payments"));break;case"report":if(item&&"0"!==item){var item={title:"Report an Issue",icon:"fa-exclamation-triangle"};"link"==issue.service?item.url=issue.link_create:item.state="report",menu.push(item),default311Url||(default311Url="/city/report")}break;case"status":if(item&&"0"!==item){var item={title:"Check Status",icon:"fa-wrench"};"link"==issue.service?item.url=issue.link_status:item.state="status",menu.push(item),default311Url||(default311Url="/city/status")}}})}};self.getDefaultUrl=function(appId){return initMenu(appId),default311Url},self.getMenu=function(appId){return initMenu(appId),menu},
// Just return self
...
// Initialize Stripe
var handler=StripeCheckout.configure({name:post.title.rendered,token:function(token,args){}});
// Submit
