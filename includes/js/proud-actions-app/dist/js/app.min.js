...
$scope.tabClick=function(tab,e){e.preventDefault(),$scope.activeTab=tab},$scope.city=$rootScope.activeCity,$timeout(function(){$scope.showMap=jQuery(".widget-proud-map-app").length,console.log($scope)},2e3),
//console.log($scope);
$scope.clickMap=function(e){jQuery("#map-wrapper").trigger("click"),jQuery("#menu-ui a:last").trigger("click"),e.preventDefault()},
// Get myIssues
$scope.hasMyIssues=!1;var local=localStorage.getItem("reportedIssues");-1===local&&($scope.hasMyIssues=-1),local&&-1!==local&&($scope.myIssues=JSON.parse(local),console.log("LOCAL",local),$scope.activeTab="me",$scope.hasMyIssues=!0),
// Get recent issues
...
//local = '[]';
-1!==local&&(local=local?JSON.parse(local):[],local.push({created_at:data.created_at,title:data.summary,id:data.id,address:data.address}),localStorage.setItem("reportedIssues",JSON.stringify(local))),
// Redirect
$state.go("city.status.item",{code:data.id,saved:-1===local?1:2})},function(err){
