...
$rootScope.$state=$state,$rootScope.$stateParams=$stateParams,$rootScope.currentSection="",$rootScope.currentRoute="",$rootScope.mapboxAccessToken=_.get(Proud,"settings.proud_311_app.mapbox_access_token")||"pk.eyJ1IjoiYWxiYXRyb3NzZGlnaXRhbCIsImEiOiI1cVUxbUxVIn0.SqKOVeohLfY0vfShalVDUw",$rootScope.mapboxMap=_.get(Proud,"settings.proud_311_app.mapbox_map")||"albatrossdigital.lpkdpcjb",$rootScope.proudShowcaseKey="325jk154hl3y8r2J34NRAasdfasdf",$rootScope.showcaseApiUrl="http://ui.dev.getproudcity.com/",//@todo: switch to my.getproudcity.com
...
$rootScope.$on("$stateChangeStart",function(event,toState,toParams,fromState,fromParams){console.log("sjsjs")}),$rootScope.$on("$stateChangeSuccess",function(event,toState,toParams,fromState,fromParams){
// send tracking
...
// listen for a click
$element.on("click",function(){jQuery("#"+$scope.openReveal).foundation("reveal","open")})}}}),angular.module("311AppParent").filter("termByTid",function(){return function(items,tid){for(var i=0;i<items.length;i++)if(parseInt(items[i].tid)==parseInt(tid))return items[i];return null}}).filter("termsNoParent",function(){return function(items){for(var filtered=[],i=0;i<items.length;i++)0==items[i].parent.length&&filtered.push(items[i]);return filtered}}).filter("termsByParent",function(){return function(items,tid){for(var filtered=[],i=0;i<items.length;i++)for(var j=0;j<items[i].parent.length;j++)items[i].parent[j].id==tid&&filtered.push(items[i]);return filtered}}),angular.module("311App",["drupalService","seeClickFixService","paymentService","ui.router","ngSanitize","ngAnimate","stripe.checkout","ngTouch","scrollTo"]).run(["$rootScope","$state","$stateParams","$window","$location",function($rootScope,$state,$stateParams,$window,$location){
// Api Options
// ---------------------------
$rootScope.apiUrl=_.get(Proud,"settings.proud_311_app.api_path")||"http://dev.getproudcity.com/",$rootScope.paymentUrl=_.get(Proud,"settings.proud_311_app.payment_url")||"http://demo.helmcivic.com/invoice-example",//$rootScope.proudcityApi + 'invoice-example';
$rootScope.trackUrl=_.get(Proud,"settings.proud_311_app.track_url")||"http://markaspot.helmcivic.com/georeport/v2",$rootScope.seeclickfixUrl=_.get(Proud,"settings.proud_311_app.seeclickfixUrl")||"https://test.seeclickfix.com/api/v2/",$rootScope.vocabularyVid=5,
// Global options
// ---------------------------
...
StripeCheckoutProvider.defaults({key:_.get(Proud,"settings.proud_311_app.payment_key")||""})}]).directive("app311Wrap",function($window,$rootScope){return{restrict:"AC",templateUrl:"views/apps/311App/app-311-wrap.html",transclude:!0,link:function($scope,$element,$attrs){
// Display options
// ---------------------------
// Expand categories?
$rootScope.appPageDisplay=_.get(Proud,"settings.proud_311_app."+$rootScope.appId+".expand_section")||!1,
// Restrict to visitor, resident, ect ?
$rootScope.categorySection=_.get(Proud,"settings.proud_311_app."+$rootScope.appId+".category_section")||!1}}}),angular.module("311App").directive("menu",function($rootScope,Menu311,$state){return{restrict:"A",templateUrl:"views/apps/311App/menu311.html",link:function($scope,$element,$attrs){
// Push to scope
...
// Only run once
active311Items||(active311Items=_.get(Proud,"settings.proud_311_app."+appId+".active_tabs")||{faq:!0,payments:!0,report:!0,status:!0},_.forEach(active311Items,function(item,key){switch(key){case"faq":item&&(menu.push({title:"Get Answers",state:"faq",icon:"fa-question-circle"}),default311Url||(default311Url="/city/answers"));break;case"payments":item&&(menu.push({title:"Make a Payment",state:"payments",icon:"fa-credit-card"}),default311Url||(default311Url="/city/payments"));break;case"report":item&&(menu.push({title:"Report an Issue",state:"report",icon:"fa-exclamation-triangle"}),default311Url||(default311Url="/city/report"));break;case"status":item&&(menu.push({title:"Check Status",state:"status",icon:"fa-wrench"}),default311Url||(default311Url="/city/status"))}}))};self.getDefaultUrl=function(appId){return initMenu(appId),default311Url},self.getMenu=function(appId){return initMenu(appId),menu},
// Just return self
...
doScroll:!1},resolve:{terms:function($stateParams,$rootScope,TaxonomyTerm){return TaxonomyTerm.query({vocabulary:$rootScope.vocabularyVid,sort:"weight",direction:"ASC"}).$promise.then(function(data){return data.list})}},controller:function($scope,$rootScope,$state,terms){$scope.terms=terms,$scope.active=null,$scope.childOpen=!1,$scope.open=function(item){$scope.active=item},$scope.openChildren=function(){$scope.childOpen=!0}}}).state("city.faq.child",{url:"/:tid",templateUrl:"views/apps/311App/faq/faq-child.html",data:{title:"FAQ",// Sets meta title
description:"About the about",// Sets different meta description
keywords:"About, this, page"},controller:function($scope,$rootScope,$state,$filter,terms,Node){$scope.activeTerm=$filter("termByTid")(terms,$state.params.tid),$scope.activeParent=$filter("termByTid")(terms,$scope.activeTerm.parent[0].id),$state.go("city.faq.child.answers",{nid:"list"})}}).state("city.faq.child.answers",{url:"/:nid",templateUrl:"views/apps/311App/faq/faq-child-answers.html",data:{title:"FAQ",// Sets meta title
description:"About the about",// Sets different meta description
keywords:"About, this, page"},controller:function($scope,$rootScope,$state,$filter,Node){$scope.activeNid="list"!=$state.params.nid?$state.params.nid:null,
// @todo: put this in resolve
Node.query({field_faq_category:$state.params.tid,sort:"title",direction:"ASC"}).$promise.then(function(data){$scope.nodes=data.list})}})}]),angular.module("311App").config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){
//.when('', '/answers');
...
// Initialize Stripe
var handler=StripeCheckout.configure({name:node.title,token:function(token,args){}});
// Submit
$scope.submit=function(payment){if(void 0!=$scope.node.field_payment_link)window.location=$scope.node.field_payment_link.replace("[invoice]",payment.number).replace("[amount]",payment.amount);else{
// Stripe checkout 
var options={description:"Invice "+payment.number,amount:100*$scope.amount};
// The rejection callback doesn't work in IE6-7.
handler.open(options).then(function(result){},function(){})}}}}).state("city.payments.success",{templateUrl:"views/apps/311App/payment/payment-success.html",url:"/:nid/:token",resolve:{node:function($stateParams,$filter,nodes){return $filter("filter")(nodes,{nid:$stateParams.nid})[0]}},controller:function($scope,$rootScope,$state,node,Payment){}})}])}(jQuery);