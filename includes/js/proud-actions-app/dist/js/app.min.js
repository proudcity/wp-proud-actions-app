...
$rootScope.trackUrl=_.get(Proud,"settings.proud_actions_app.global.track_url")||"http://markaspot.helmcivic.com/georeport/v2",$rootScope.seeclickfixUrl=_.get(Proud,"settings.proud_actions_app.global.seeclickfixUrl")||"https://test.seeclickfix.com/api/v2/",$rootScope.issue=_.get(Proud,"settings.proud_actions_app.global.issue")||"http://markaspot.helmcivic.com/georeport/v2",$rootScope.payment=_.get(Proud,"settings.proud_actions_app.global.payment")||"http://markaspot.helmcivic.com/georeport/v2",
// Global options
// ---------------------------
...
// Only run once
active311Items||(active311Items=_.get(Proud,"settings.proud_actions_app.instances."+appId+".active_tabs")||{faq:!0,payments:!0,report:!0,status:!0},_.forEach(active311Items,function(item,key){switch(key){case"faq":item&&"0"!==item&&(menu.push({title:"Get Answers",state:"faq",icon:"fa-question-circle"}),default311Url||(default311Url="/city/answers"));break;case"payments":item&&"0"!==item&&(menu.push({title:"Make a Payment",state:"payments",icon:"fa-credit-card"}),default311Url||(default311Url="/city/payments"));break;case"report":if(item&&"0"!==item){var item={title:"Report an Issue",icon:"fa-exclamation-triangle"};"link"==$rootScope.issue.service?item.url=$rootScope.issue.link_report:item.state="report",menu.push(item),default311Url||(default311Url="/city/report")}break;case"status":if(item&&"0"!==item){var item={title:"Check Status",icon:"fa-wrench"};"link"==$rootScope.issue.service?item.url=$rootScope.issue.link_report:item.state="report",menu.push(item),default311Url||(default311Url="/city/status")}}}))};self.getDefaultUrl=function(appId){return initMenu(appId),default311Url},self.getMenu=function(appId){return initMenu(appId),menu},
// Just return self
