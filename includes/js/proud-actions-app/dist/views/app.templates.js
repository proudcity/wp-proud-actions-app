angular.module('311AppParent').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/apps/311App/faq/faq-child-answers.html',
    "<div ng-repeat=\"node in nodes\" class=\"panel\"><div class=\"panel-heading\"><h3 class=\"panel-title\"><a href=\"\" ui-sref=\"city.faq.child.answers({nid: node.nid})\">{{node.title}}</a></h3></div><div class=\"panel-body\" ng-show=\"activeNid == node.nid && node.body\"><div ng-bind-html=\"node.body.value\"></div><div proudcity-get-involved></div><p ng-if=\"node.field_faq_link\"><a href=\"\" ng-href=\"{{node.field_faq_link.url}}\" class=\"btn btn-primary\">{{node.field_faq_link.title}} &raquo;</a></p></div></div><div main-toggle=\"slide-left\" main-toggle-force=\"add\" main-toggle-enter=\"true\"></div>"
  );


  $templateCache.put('views/apps/311App/faq/faq-child.html',
    "<ol class=\"breadcrumb clean\"><li><a href=\"\" main-toggle=\"slide-left\"><i class=\"fa fa-th\"></i><span class=\"sr-only\">Select category</span></a></li><li class=\"active\"><i class=\"fa fa-search\"></i> Find Answer</li></ol><div class=\"row\"><div class=\"col-sm-9\"><div class=\"panel-group\" ui-view></div></div><div class=\"col-sm-3\"><h5>{{activeParent.name}}</h5><ul class=\"nav nav-pills nav-stacked\"><li ng-repeat=\"item in terms | termsByParent:activeParent.tid | orderBy: 'weight'\" ng-class=\"item.tid == activeTerm.tid ? 'active' : ''\"><a href=\"\" ui-sref=\"city.faq.child.answers({tid: item.tid, nid: 'list'})\">{{item.name}}</a></li></ul></div></div>"
  );


  $templateCache.put('views/apps/311App/faq/faq.html',
    "<div class=\"slide-left-2 column\"><ol class=\"breadcrumb clean\"><li class=\"active\"><i class=\"fa fa-th\"></i> Select answer type</li></ol><dl class=\"angular-311-2col\"><dd ng-repeat=\"item in terms | termsNoParent | orderBy: 'weight'\" class=\"col-sm-6 col-md-4\" ng-class=\"{'active' : item.tid == active.tid, 'expanded': $root.displayExpanded('answers')}\"><h4><a ng-click=\"open(item)\" href=\"\" ng-click=\"open(item)\" style=\"color: {{item.field_icon.color}}\"><i ng-if=\"item.field_icon\" class=\"fa {{item.field_icon.icon}}\"></i><span>{{item.name}}</span></a></h4><div class=\"animation-wrap\"><div class=\"slidedown\" ng-if=\"active.tid == item.tid || $root.displayExpanded('answers')\"><!-- @todo: ng-repeat=\"child in terms | filter: { parent: [{id: item.tid}] } |  orderBy: 'weight'\" --><ul class=\"taxonomy-vocabulary-2col\"><li ng-repeat=\"child in terms | termsByParent:item.tid\"><a main-toggle=\"slide-left\" href=\"\" ui-sref=\"city.faq.child.answers({tid: child.tid, nid: 'list'})\">{{child.name}}</a></li></ul></div></div></dd></dl></div><div class=\"animation-wrap slide-left-2\"><div class=\"quickfade\" ui-view></div></div>"
  );
